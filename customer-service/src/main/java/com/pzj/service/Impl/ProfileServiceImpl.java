...
public class ProfileServiceImpl implements ProfileService {
	...
	private Boolean modifyCustomerAccoutState(Long id, Long operator, Integer state1, Integer state2) {
		...
		if (update == 1) {
			sendModifyCustomer(id, state1, state2, operator);
			return true;
		}
		return false;
	}

	private void sendModifyCustomer(Long customerId, Integer oldStatus, Integer newStaus, Long operatorId) {
		if (CheckUtils.isNotNull(customerId) && CheckUtils.isNotNull(oldStatus) && CheckUtils.isNotNull(newStaus)
				&& CheckUtils.isNotNull(operatorId)) {
			ModifyCustomer modifyCustomer = new ModifyCustomer(customerId, oldStatus, newStaus, operatorId, new Date());
