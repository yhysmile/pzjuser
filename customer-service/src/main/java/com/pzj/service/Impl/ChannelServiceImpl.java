...
public class ChannelServiceImpl extends BaseUserServiceImpl<SysChannel, SysChannelMapper> implements IChannelService {
	...
	public Result<QueryResult<SysChannel>> queryChannelContainUser(SysChannel channelParam, SysUser distributorParam,
		...
		List<SysChannel> channelList = mapper.selectChannelContainUser(channelParam, paramUser, null, tmpDelIds,
				tmpAddIds);
		...
		int count = mapper.countChannelContainUser(channelParam, distributorParam, tmpDelIds, tmpAddIds);
		if (count == 0) {
			return new Result<>(queryResult);
		}
		channelList = mapper.selectChannelContainUser(channelParam, distributorParam, pageModel, tmpDelIds, tmpAddIds);
	...
	@Override
	public Result<QueryResult<SysChannel>> queryChannelsUserNotJoin(SysChannel channelParam, SysUser distributorParam,
			PageModel pageModel, List<Long> channelIds) {
		QueryResult<SysChannel> queryResult = new QueryResult<>(pageModel.getPageNo(), pageModel.getPageSize());

		//查询满足渠道条件的所有渠道，并且not in 已经关联的
		List<SysChannel> channelList = mapper.queryChannelsUserNotJoin(channelParam, null, null, channelIds);
		...
		distributorParam.setId(null);
		int count = mapper.countChannelContainNotUser(paramCh, distributorParam, channelIds);
		if (count == 0) {
			return new Result<>(queryResult);
		}
		channelList = mapper.queryChannelsUserNotJoin(paramCh, distributorParam, pageModel, channelIds);
		queryResult.setRecords(channelList);
		queryResult.setTotal(count);
		return new Result<>(queryResult);
	}
