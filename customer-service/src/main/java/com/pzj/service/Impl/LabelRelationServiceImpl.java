...
import com.pzj.core.customer.dao.SysLabelRelationMapper;
...
public class LabelRelationServiceImpl extends BaseRelationshipServiceImpl<SysLabelRelationKey, SysLabelRelationMapper>
	...
	@Override
	public Long insertBatch(List<SysLabelRelationKey> entityList) {
		...
		if (idGenerater != null) {
			for (SysLabelRelationKey entity : entityList) {
		...
		if (insertBatch > 0 && entityList != null && !entityList.isEmpty()){
			SysLabelRelationKey sysLabelRelationKey = entityList.get(0);
			if (sysLabelRelationKey != null && sysLabelRelationKey.getRelType() != null
					&& (sysLabelRelationKey.getRelType().equals("channelUser") ||
						sysLabelRelationKey.getRelType().equals("directChannleuser"))){

				Date currentDate = new Date();
				DirectChannelUser addCustomerMsg = new DirectChannelUser();
				addCustomerMsg.setOperatingDate(currentDate);

				if (sysLabelRelationKey.getsId() != null){
					addCustomerMsg.setSupplierId(sysLabelRelationKey.getsId());
				}

				List<Relation> relationses = new ArrayList<>(entityList.size());

				for (SysLabelRelationKey key : entityList){
					Relation relation = new Relation();
					relation.setChannelId(Long.valueOf(key.getObjId()));
					relation.setCustomerId(Long.valueOf(key.getRelId()));
					relationses.add(relation);
				}
	...
	public Long deleteBatchSelective(List<SysLabelRelationKey> recordList) {
		Long num = 0l;
		try {
			...
			if (num > 0 && recordList != null && !recordList.isEmpty()) {
				SysLabelRelationKey sysLabelRelationKey = recordList.get(0);
				if (sysLabelRelationKey != null && sysLabelRelationKey.getRelType() != null
						&& (sysLabelRelationKey.getRelType().equals("channelUser") ||
						sysLabelRelationKey.getRelType().equals("directChannleuser"))) {
					...
					List<Relation> relationses = new ArrayList<>(recordList.size());

					for (SysLabelRelationKey key : recordList) {
						Relation relation = new Relation();
						relation.setChannelId(Long.valueOf(key.getObjId()));
						relation.setCustomerId(Long.valueOf(key.getRelId()));
						relationses.add(relation);
					...
					channelMqMessage.sendDelCustomerMsg(addCustomerMsg);
				}
			}
		} catch (Exception e) {
