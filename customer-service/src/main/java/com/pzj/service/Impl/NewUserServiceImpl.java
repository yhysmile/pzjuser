...
import com.pzj.core.customer.profile.*;
...
public class NewUserServiceImpl implements DistributorService {
	...
	public Result<QueryResult<QueryCustomerResponse>> queryDistributorsOfMaster(QueryCustomerRequest param,
		...
		com.pzj.core.customer.commons.PageBean pageBeanParam = QueryUtil.defaultPageBean(page);
	...
	public Result<Long> createDistributor(final CreateCustomerRequest distributor) {
		...
		Result<Long> result = new RpcCaller<Long>(){
			...
			public Long call() {
				...
				// 是否需要发送密码的短信
				if (CheckUtils.isNotNull(result.getPassWordInfo())
						&& CheckUtils.isNotNull(result.getPassWordInfo().getPassWord())) {
					// 发送短信
					sendPasswordSM(distributor, result.getPassWordInfo().getPassWord());
				}
